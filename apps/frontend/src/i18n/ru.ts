export const ru = {
  'header.tagline': 'будь на связи с тренером',
  'header.checking': 'Проверяем...',
  'header.greeting': 'Привет, {name}',
  'header.dashboard': 'Дэшборд атлета',
  'header.logout': 'Выйти',
  'header.login': 'Войти',

  'auth.checking': 'Проверяем авторизацию...',

  'login.title': 'Вход',
  'login.subtitle': 'Введите email и пароль.',
  'login.invalid': 'Неверные данные.',
  'login.failed': 'Не удалось войти. Попробуйте еще раз.',
  'login.button': 'Войти',
  'login.loading': 'Входим...',
  'login.noAccount': 'Нет аккаунта?',
  'login.createOne': 'Создать',
  'login.field.email': 'Email',
  'login.field.password': 'Пароль',
  'login.placeholder.email': 'email@abc.com',
  'login.placeholder.password': 'some123PASSWORD',

  'register.title': 'Создать аккаунт',
  'register.subtitle': 'Регистрация по email, имени пользователя и паролю.',
  'register.loading': 'Создаем аккаунт...',
  'register.button': 'Создать аккаунт',
  'register.failed': 'Не удалось зарегистрироваться.',
  'register.field.email': 'Email',
  'register.field.username': 'Имя пользователя',
  'register.field.password': 'Пароль',
  'register.placeholder.email': 'email@abc.com',
  'register.placeholder.username': 'newUserName',
  'register.placeholder.password': 'some123PASSWORD',

  'dashboard.title': 'Дэшборд атлета',
  'dashboard.subtitle': 'Следи за прогрессом и отмечай тренировки.',
  'dashboard.createJournal': 'Создать журнал',
  'dashboard.loading': 'Загружаем дэшборд...',
  'dashboard.errorLoad': 'Не удалось загрузить дэшборд.',
  'dashboard.errorCreateRecord': 'Не удалось создать запись.',
  'dashboard.emptyTitle': 'Журналов пока нет',
  'dashboard.emptyAction': 'Создай',
  'dashboard.emptyText': 'тренировочный журнал, чтобы видеть прогресс и записи.',
  'dashboard.yourJournals': 'Твои журналы',
  'dashboard.lastResult': 'Последний результат',
  'dashboard.noResult': 'Нет результата',
  'dashboard.missingRecord': 'Сегодня запись еще не сделана. Заполни сейчас!',
  'dashboard.field.result': 'Результат',
  'dashboard.field.coachNotes': 'Заметки тренера',
  'dashboard.field.privateNotes': 'Личные заметки',
  'dashboard.placeholder.result': 'Например 612.5',
  'dashboard.placeholder.coachNotes': 'Что сказал тренер',
  'dashboard.placeholder.privateNotes': 'Что отметить для себя',
  'dashboard.save': 'Записать тренировку',
  'dashboard.saving': 'Сохраняем...',
  'dashboard.openJournal': 'Открыть журнал',
  'dashboard.nextTitle': 'Что дальше?',
  'dashboard.nextText': 'Открой журнал, чтобы увидеть все записи и либо создать новую.',
  'dashboard.goToJournal': 'Перейти к журналу',
  'dashboard.addRecord': 'Сделать текущую запись',

  'journal.titleFallback': 'Журнал',
  'journal.back': 'Назад к дэшборду',
  'journal.addRecord': 'Добавить запись',
  'journal.loading': 'Загружаем журнал...',
  'journal.errorLoad': 'Не удалось загрузить журнал.',
  'journal.latestTitle': 'Последние 3 записи',
  'journal.latestEmpty': 'Пока нет записей.',
  'journal.allTitle': 'Все записи',
  'journal.allEmpty': 'Записей пока нет.',
  'journal.noDate': 'Нет даты',
  'journal.noResult': 'Нет результата',
  'journal.coach': 'Тренер',
  'journal.private': 'Личное',
  'journal.dash': '—',
  'journal.prev': 'Назад',
  'journal.next': 'Вперед',
  'journal.page': 'Страница {page} из {pages}',

  'record.title': 'Новая запись',
  'record.backJournal': 'Назад к журналу',
  'record.backDashboard': 'Назад к дэшборду',
  'record.field.result': 'Результат',
  'record.field.coachNotes': 'Заметки тренера',
  'record.field.privateNotes': 'Личные заметки',
  'record.placeholder.result': 'Например 612.5',
  'record.placeholder.coachNotes': 'Ключевые комментарии тренера',
  'record.placeholder.privateNotes': 'Что важно запомнить',
  'record.save': 'Сохранить запись',
  'record.saving': 'Сохраняем...',
  'record.errorCreate': 'Не удалось создать запись.',

  'journalCreate.title': 'Новый журнал',
  'journalCreate.back': 'Назад к дэшборду',
  'journalCreate.field.sportType': 'Вид спорта',
  'journalCreate.option.shooting': 'Стрельба',
  'journalCreate.create': 'Создать журнал',
  'journalCreate.creating': 'Создаем...',
  'journalCreate.error': 'Не удалось создать журнал.',
} as const;
